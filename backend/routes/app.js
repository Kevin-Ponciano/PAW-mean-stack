...
router.post('/genero/criar', async (req, res, next) => {
    const genero = new Genero({
        tipo_genero: req.body.tipo_genero
    });
...
router.get('/generos/detalhar', async (req, res, next) => {
  try{
    const lista_generosRecuperado = await Genero.find({}).exec();
    res.render('genero_detalhes_pagina', { titulo: 'Detalhes do Gênero', lista: lista_generosRecuperado, listaExiste: false });
  }
  catch(err){
    err.stack;
    return next(err);
...
router.post('/generos/detalhar',  async (req, res, next) => {
  try {
...
router.get('/livros/ordenar', async (req, res, next) => {
    try {
...
router.get('/livros/detalhes/:id', async (req, res, next) => {
    try{
...
router.get('/livros/detalhes/titulo/:titulo', async (req, res, next) => {
    try{
